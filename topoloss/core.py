...
class TopoLoss:
    ...
    def compute(self, model, reduce_mean = False):
        layer_wise_losses = {}

        for loss_info in self.losses:
            layer = get_layer_by_name(
                model=model,
                layer_name=loss_info.layer_name
            )
            self.check_layer_type(layer=layer)

            if isinstance(layer, nn.Linear):
                cortical_sheet = get_weight_cortical_sheet_linear(
                    layer=layer
                )
            else:
                cortical_sheet = get_cortical_sheet_conv(
                    layer=layer
                )
