...
def test_loss_linear(num_steps: int, hidden_size: int, init_from_layer: bool):  # num_steps is now passed by the fixture

    model = nn.Sequential(
        nn.Linear(30, hidden_size),  # 0
        nn.ReLU(),
        nn.Linear(30, 20)  # 2
    )
    ...
    if init_from_layer:
        tl = TopoLoss(
            losses=[
                LaplacianPyramidLoss.from_layer(model=model, layer=model[0], scale = 1.0, factor_h=3.0, factor_w=3.0),
                LaplacianPyramidLoss.from_layer(model=model, layer=model[2], scale = 1.0, factor_h=3.0, factor_w=3.0),
            ]
        )
    else:
    ...
    assert losses[-1] < losses[0], f"Expected loss to go down for {num_steps} training steps, but it did not. \x1B[3msad sad sad\x1B[23m"
